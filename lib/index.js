...
function listPlaylists(playlists) {
  playlists.forEach(playlist => {
    const playlistHtml = `<li id="playlist-${playlist.id}">Playlist Name: ${playlist.playlist_name}</li>`
...
function listSongs(songs, playlist) {
  songs.forEach(song => {
    const songHtml = `<li id="${song.id}">Track Title: ${song.name} Artist: ${song.artist_name} Rating: ${song.rating}</li>`
    document.getElementById('playlist-'+playlist).insertAdjacentHTML('beforeend', songHtml)
  })
}
