...
function listPlaylists(playlists) {
  playlists.forEach(playlist => {
    const playlistHtml = `<button class="accordion" id="playlist-${playlist.id}">${playlist.playlist_name}</button><div class="panel" id="songs-panel-${playlist.id}"></div>`
...
document.querySelector(DOMstrings.favoritesList).addEventListener('click', function(event) {
  if (event.target.className === 'add-btn btn' ) {
    const rawHtml = event.path[1].innerHTML;
    const favoriteId = event.path[1].id;
    getCleanFavorite(favoriteId, rawHtml);
    // songRaw = event.path[1].innerHTML.split('<')
    // newSongRaw = songRaw[0] + songRaw[1] + songRaw[2]
    // newSongRaw = newSongRaw.replace('br> By: ', ',')
    // newSongRaw = newSongRaw.replace('br> By: ', ',')

    // appendPlaylist(event.path[1])
  }
})

function getCleanFavorite(id, rawHtml) {
  songRaw = rawHtmlHtml.split('<');
  newSongRaw = songRaw[0] + songRaw[1] + songRaw[2];
  newSongRaw = newSongRaw.replace('br> By: ', ',');
  newSongRaw = newSongRaw.replace('br> By: ', ',');
  appendPlaylist(id, newSongRaw[0], newSongRaw[1], newSongRaw[2])
}

function appendPlaylist(id, name, artistName, rating, playlistId) {
  const newSongHtml = `<div id="${id}">Track Title: ${name} Artist: ${artist_name} Rating: ${rating}</div>`
  document.getElementById('songs-panel-'+playlist).insertAdjacentHTML('beforeend', newSongHtml)
}
