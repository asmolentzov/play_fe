...
function appendFavorites(favorite) {
  const favoriteHtml = `<li id="${favorite.id}">${favorite.name} By: ${favorite.artist_name} <button class="remove-btn" id="remove-${favorite.id}">Remove</button></li>`
...
$(document).ready(function() {
  $('.remove-btn').on('click', function(event) {
    const favoriteId = this.id.split('-')[1]
