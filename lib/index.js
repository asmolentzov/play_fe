...
document.querySelector(DOMstrings.searchResults).addEventListener('click', function(event) {
    ...
    fetch(process.env.BASE_URL + `/api/v1/favorites`, {
      method: 'POST',
...
(function() {
  fetch(process.env.BASE_URL + '/api/v1/playlists')
    .then((response) => response.json())
...
function listPlaylists(playlists) {
  ...
  sortedPlaylists.forEach((playlist, index) => {
    const playlistHtml = `<button class="remove-btn btn" id="remove-${playlist.playlist_name}">Remove</button><button class="accordion" id="playlist-${playlist.id}">${playlist.playlist_name}</button><div class="panel" id="songs-panel-${playlist.id}"></div>`
...
function listSongs(songs, playlist) {
  songs.forEach(song => {
    const songHtml = `<div class="playlist-song" id="${song.id}">${song.name}<br>Artist: ${song.artist_name}<br>Rating: ${song.rating}</div>`
...
document.querySelector(DOMstrings.updatePlaylistBtn).addEventListener('click', function(event) {
  let activePlaylistRaw = document.querySelector('.active');
  let activePlaylistId = activePlaylistRaw.id.split('-');
  let newPlaylistTitle = document.querySelector(DOMstrings.updatePlaylistForm).value;
...
function appendPlaylist(id, name, artistName, rating, playlistId) {
  const newSongHtml = `<div class="playlist-song" id="${id}">${name}<br>Artist: ${artistName}<br>Rating: ${rating}</div>`
...
function postFavoriteToPlaylist(favoriteSongId, playlistId) {
  fetch(process.env.BASE_URL + `/api/v1/playlists/${playlistId}/favorites/${favoriteSongId}`, {
    method: 'POST',
    headers: { 'Content-Type':
      'application/json'},
    body: JSON.stringify({
...
document.querySelector(DOMstrings.playlistsList).addEventListener('click', function(event) {
    if (event.target.className === 'accordion' || event.target.className === 'accordion active') {
      ...
      let playlistRaw = event.target.nextElementSibling;
      let playlistId = playlistRaw.id.split('-');
...
function removeFavorite(event) {
  if(event.target.innerHTML === "Remove") {
    ...
    fetch(process.env.BASE_URL + '/api/v1/favorites/' + favoriteId, {
      method: 'DELETE'
    })
    .then()
    .catch(error => {
